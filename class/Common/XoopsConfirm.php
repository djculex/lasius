...
namespace XoopsModules\Lasius\Common;
...
class XoopsConfirm
{
    ...
    public function getFormXoopsConfirm()
    {
        ...
        if (!defined('CO_LASIUS_DELETE_CONFIRM')) {
            \define('CO_LASIUS_DELETE_CONFIRM', 'Confirm delete');
            \define('CO_LASIUS_DELETE_LABEL', 'Do you really want to delete:');
        }
        ...
        if ('' === $this->title) {
            $this->title = CO_LASIUS_DELETE_CONFIRM;
        }
        if ('' === $this->label) {

            $this->label = CO_LASIUS_DELETE_LABEL;
        }
