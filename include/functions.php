...
function lasiusMetaKeywords($content)
{
...
function lasiusMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function lasius_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = lasius_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function lasius_Filter($url, $type = '') {
